...
<p>\
Служба HPKMCU запущена і приймає підключення.\
</p>\
<p>\
Конференція, що створена підключенням першого терміналу, спочатку працює у некерованому режимі, при цьому всі учасники конференції бачать однакову картинку.\
 У некерованому режимі HPKMCU автоматично змінює розкладку екрану залежно від кількості підключених користувачів, у порядку, визначеному у файлі layouts.conf.\
...
window.l_filtermode = [
    "Режим фільтрації зображення при зміні розміру кадрів: НАЙШВИДШИЙ.", "Режим фільтрації зображення при зміні розміру кадрів: ОПТИМАЛЬНИЙ (білінійна інтерполяція).", "Режим фільтрації зображення при зміні розміру кадрів: НАЙБІЛЬШ ЯКІСНИЙ."
];
...
window.l_server_id = "Ідентифікатор сервера HPKMCU";
